 ...
 * Development templates are displayed when the application is in dev mode (i.e. craft()->config->get('devMode') =
 * true). Detailed error information with source code are displayed in these templates. Production templates are meant
 * to be shown to end-users and are used when the application is in production mode. For security reasons, they only
 * display the error message without any sensitive information. ErrorHandler looks for the templates from the following
 * locations in order:
 ...
 * {@link maxSourceLines} can be changed to specify the number of source code lines to be displayed in development
 * views. ErrorHandler is a core application component that can be accessed via {@link
