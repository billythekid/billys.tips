 ...
 * @property boolean               $isNewRecord    Whether the record is new and should be inserted when calling {@link
 *           save}. This property is automatically set in constructor and {@link populateRecord}. Defaults to false,
 *           but it will be set to true if the instance is created using the new operator.
 * @property mixed                 $primaryKey     The primary key value. An array (column name=>column value) is
 *           returned if the primary key is composite. If primary key is not defined, null will be returned.
...
abstract class CActiveRecord extends CModel
{
     ...
     *
     * @param integer                           $duration   the number of seconds that query results may remain valid
     *                                                      in cache. If this is 0, the caching will be disabled.
     ...
     * @param integer                           $queryCount number of SQL queries that need to be cached after calling
     *                                                      this method. Defaults to 1, meaning that the next SQL query
     *                                                      will be cached.
     ...
     *                         If the current record is not a new record and it does not have the related objects
     *                         loaded they will be retrieved from the database even if this is set to false. If the
     *                         current record is a new record and this value is false, the related objects will not be
     *                         retrieved from the database.
     * @param mixed   $params  array or CDbCriteria object with additional parameters that customize the query
     *                         conditions as specified in the relation declaration. If this is supplied the related
     *                         record(s) will be retrieved from the database regardless of the value or {@link
     *                         $refresh}. The related record(s) retrieved when this is supplied will only be returned
     *                         by this method and will not be loaded into the current record's relation. The value of
     *                         the relation prior to running this method will still be available for the current record
     *                         if this is supplied.
     ...
     * Note, for composite foreign keys, they can be either listed together, separated by commas or specified as an
     * array in format of array('key1','key2'). In case you need to specify custom PK->FK association you can define it
     * as array('fk'=>'pk'). For composite keys it will be array('fk_c1'=>'pk_с1','fk_c2'=>'pk_c2'). For foreign keys
     * used in MANY_MANY relation, the joining table must be declared as well
     ...
     *   Column names should be disambiguated if they appear in an expression (e.g. COUNT(relationName.name) AS
     *   name_count).</li>
     ...
     *   be disambiguated with prefix 'relationName.' (e.g. relationName.age). This option only applies to HAS_MANY and
     *   MANY_MANY relations.</li>
     ...
     * <li>'through': name of the model's relation that will be used as a bridge when getting related data. Can be set
     * only for HAS_ONE and HAS_MANY. This option has been available since version 1.1.7.</li>
     * </ul>
     ...
     *                          was successfull but no attributes had changed and the database driver returns 0 for the
     ...
     *
     * @return boolean whether the row still exists in the database. If true, the latest data will be populated to this
     *                 active record.
     */
     ...
     *               composite. If primary key is not defined, null will be returned.
     */
     ...
     *
     * @return mixed the old primary key value. An array (column name=>column value) is returned if the primary key is
     *               composite. If primary key is not defined, null will be returned.
     * @since 1.1.0
     */
     ...
     *                         object; Otherwise, it should be an instance of {@link CDbCriteria}.
     ...
     * @param mixed $condition query condition or criteria.
     * @param array $params    parameters to be bound to an SQL statement.
     * @return static[] list of active records satisfying the specified condition. An empty array is returned if none
     *                  is found.
     */
     ...
     * See {@link find()} for detailed explanation about $condition and $params.
     *
     * @param array $attributes list of attribute values (indexed by attribute names) that the active records should
     *                          match. An attribute value can be an array which will be used to generate an IN
     *                          condition.
     * @param mixed $condition  query condition or criteria.
     * @param array $params     parameters to be bound to an SQL statement.
     * @return static the record found. Null if none is found.
     ...
     * @param mixed $condition query condition or criteria.
     * @param array $params    parameters to be bound to an SQL statement.
     * @return string the number of rows satisfying the specified query condition. Note: type is string to keep max.
     *                precision.
     */
     ...
     * @param string  $index         the name of the attribute whose value will be used as indexes of the query result
     *                               array. If null, it means the array will be indexed by zero-based integers.
...
class CActiveRelation extends CBaseActiveRelation
{
    ...
    public $alias;
    /**
     * @var string|array specifies which related objects should be eagerly loaded when this related object is lazily
     *      loaded. For more details about this property, see {@link CActiveRecord::with()}.
     */
    public $with = array();
    /**
...
{
    /**
     * @var integer limit of the rows to be selected. It is effective only for lazy loading this related object.
     *      Defaults to -1, meaning no limit.
     */
    public $limit = -1;
    /**
     * @var integer offset of the rows to be selected. It is effective only for lazy loading this related object.
     *      Defaults to -1, meaning no offset.
     */
    public $offset = -1;
    /**
