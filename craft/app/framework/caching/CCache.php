...
abstract class CCache extends CApplicationComponent implements ICache, ArrayAccess
{
    ...
    public $keyPrefix;
    /**
     * @var boolean whether to md5-hash the cache key for normalization purposes. Defaults to true. Setting this
     *      property to false makes sure the cache key will not be tampered when calling the relevant methods {@link
     *      get()}, {@link set()}, {@link add()} and {@link delete()}. This is useful if a Yii application as well as
     *      an external application need to access the same cache pool (also see description of {@link keyPrefix}
     *      regarding this use case). However, without normalization you should make sure the affected cache backend
     *      does support the structure (charset, length, etc.) of all the provided cache keys, otherwise there might be
     *      unexpected behavior.
     * @since 1.1.11
     **/
    public $hashKey = true;
     ...
     * @return mixed the value stored in cache, false if the value is not in the cache, expired or the dependency has
     *               changed.
     */
     ...
     * @param integer          $expire     the number of seconds in which the cached value will expire. 0 means never
     *                                     expire.
