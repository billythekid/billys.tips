...
class CSecurityManager extends CApplicationComponent
{
     ...
     * This will be passed as the first parameter to {@link http://php.net/manual/en/function.mcrypt-module-open.php
     * mcrypt_module_open}. This property can also be configured as an array. In this case, the array elements will be
     * passed in order as parameters to mcrypt_module_open. For example, <code>array('rijndael-128', '', 'ofb',
     * '')</code>. Defaults to AES Note: MCRYPT_RIJNDAEL_192 and MCRYPT_RIJNDAEL_256 are *not* AES-192 and AES-256. The
     * numbers of the MCRYPT_RIJNDAEL constants refer to the block size, whereas the numbers of the AES variants refer
     * to the key length. AES is Rijndael with a block size of 128 bits and a key length of 128 bits, 192 bits or 256
     * bits. So to use AES in Mcrypt, you need MCRYPT_RIJNDAEL_128 and a key with 16 bytes (AES-128), 24 bytes
     * (AES-192) or 32 bytes (AES-256). The other two Rijndael variants in Mcrypt should be avoided, because they're
     * not standardized and have been analyzed much less than AES.
     * @since 1.1.3
     */
     ...
     * @param string $key  the decryption key. This defaults to null, meaning using {@link getEncryptionKey
     *                     EncryptionKey}.
     ...
     * @param string $key  the private key to be used for generating HMAC. Defaults to null, meaning using {@link
     *                     validationKey}.
     ...
     * @param string|null $key           the private key to be used for generating HMAC. Defaults to null, meaning
     *                                   using
     *                                   {@link validationKey} value.
     * @param string|null $hashAlgorithm the name of the hashing algorithm to be used.
     *                                   See {@link http://php.net/manual/en/function.hash-algos.php hash-algos} for
     *                                   the list of possible hash algorithms. Note that if you are using PHP 5.1.1 or
     *                                   below, you can only use 'sha1' or 'md5'. Defaults to null, meaning using
     *                                   {@link hashAlgorithm} value.
     ...
     *                                         Wikipedia}. However, in some runtime environments, PHP has no access to
     *                                         a CS-PRNG, in which case the method returns false if
     *                                         $cryptographicallyStrong is true. When $cryptographicallyStrong is
     *                                         false, the method always returns a pseudorandom result but may fall back
     *                                         to using {@link generatePseudoRandomBlock}. This method does not
     ...
     *                                         therefore expect non-blocking behavior, unlike, for example, reading
     *                                         from /dev/random on Linux, see
     ...
     * @param string       $data   data to be decrypted.
     * @param string       $key    the decryption key. This defaults to null, meaning the key should be loaded from
     *                             persistent storage.
