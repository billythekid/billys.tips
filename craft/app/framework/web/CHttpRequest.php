 ...
 *           script and before the question mark. Note, the returned pathinfo is decoded starting from 1.1.4. Prior to
 *           1.1.4, whether it is decoded or not depends on the server configuration
 *          (in most cases it is not decoded).
 * @property string                          $requestUri           The request URI portion for the currently requested
 *           URL.
 * @property string                          $queryString          Part of the request URL that is after the question
 *           mark.
 * @property boolean                         $isSecureConnection   If the request is sent via secure channel (https).
 * @property string                          $requestType          Request type, such as GET, POST, HEAD, PUT, PATCH,
 *           DELETE.
 ...
 * @property boolean                         $isFlashRequest       Whether this is an Adobe Flash or Adobe Flex
 *           request.
 ...
 * @property array                           $preferredAcceptType  The user preferred accept type as an array map, e.g.
 *           array('type' => 'application', 'subType' => 'xhtml', 'baseType' => 'xml', 'params' => array('q' => 0.9)).
 * @property array                           $preferredAcceptTypes An array of all user accepted types (as array maps
 *           like array('type' => 'application', 'subType' => 'xhtml', 'baseType' => 'xml', 'params' => array('q' =>
 *           0.9)) ) in order of preference.
 * @property string                          $preferredLanguage    The user preferred language.
 * @property array                           $preferredLanguages   An array of all user accepted languages in order of
 *           preference.
...
class CHttpRequest extends CApplicationComponent
{
     ...
     *
     * @param string $schema schema to use (e.g. http, https). If empty, the schema used for the current request will
     *                       be used.
     * @return string schema and hostname part (with port number if needed) of the request URL (e.g.
     ...
     * This method is an improved variant of the native urldecode() function and used in {@link getPathInfo
     * getPathInfo()} to decode the path part of the request URI. You may override this method to change the way the
     * path info is being decoded.
     *
     ...
     * Each array entry consists of a map with the type, subType, baseType and params, an array map of key-value
     * parameters, obligatorily including a `q` value (i.e. preference ranking) as a double. For example, an Accept
     * header value of <code>'application/xhtml+xml;q=0.9;level=1'</code> would give an array entry of
     * <pre>
     * array(
     *        'type' => 'application',
     *        'subType' => 'xhtml',
     ...
     * If the header text includes quoted strings containing space or the , or ; characters then the results may not be
     * correct! See also {@link http://tools.ietf.org/html/rfc2616#section-14.1} for details on Accept header.
     *
     ...
     * @return integer -1, 0 or 1 if $a has respectively greater preference, equal preference or less preference than
     *                 $b (higher preference comes first).
     */
     ...
     * Each array entry consists of a map with the type, subType, baseType and params, an array map of key-value
     * parameters. See {@link parseAcceptHeader()} for a description of the array map.
     *
     ...
     *                         If empty, this method will return the first language returned by
     *                         [[getPreferredLanguages()]].
     * @return string the language that the application should use. false is returned if both
     *                [[getPreferredLanguages()]] and `$languages` are empty.
     */
     ...
     * @param string  $mimeType  mime type of the content. If null, it will be guessed automatically based on the given
     *                           file name.
     ...
     * <li>Lighttpd v1.4: {@link http://redmine.lighttpd.net/projects/lighttpd/wiki/X-LIGHTTPD-send-file
     * X-LIGHTTPD-send-file}</li>
     * <li>Lighttpd v1.5: {@link http://redmine.lighttpd.net/projects/lighttpd/wiki/X-LIGHTTPD-send-file
     * X-Sendfile}</li>
     * <li>Nginx: {@link http://wiki.nginx.org/XSendfile X-Accel-Redirect}</li>
     * <li>Cherokee: {@link http://www.cherokee-project.com/doc/other_goodies.html#x-sendfile X-Sendfile and
     * X-Accel-Redirect}</li>
     * </ul>
     ...
     * an error message like this: "Internet Explorer was not able to open this Internet site. The requested site is
     * either unavailable or cannot be found.". You can work around this problem by removing the
     ...
     *                         <li>mimeType: mime type of the file, if not set it will be guessed automatically based
     *                         on the file name, if set to null no content-type header will be sent.</li>
     *                         <li>xHeader: appropriate x-sendfile header, defaults to "X-Sendfile"</li>
     *                         <li>terminate: whether to terminate the current application after calling this method,
     *                         defaults to true</li>
     *                         <li>forceDownload: specifies whether the file will be downloaded or shown inline,
     *                         defaults to true. (Since version 1.1.9.)</li>
     *                         <li>addHeaders: an array of additional http headers in header-value pairs (available
     *                         since version 1.1.10)</li>
     *                         </ul>
     */
