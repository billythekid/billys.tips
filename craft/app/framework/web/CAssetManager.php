...
/**
 * CAssetManager is a Web application component that manages private files (called assets) and makes them accessible by
 * Web clients. It achieves this goal by copying assets to a Web-accessible directory and returns the corresponding URL
 * for accessing them. To publish an asset, simply call {@link publish()}. The Web-accessible directory holding the
 * published files is specified by {@link setBasePath basePath}, which defaults to the "assets" directory under the
 * directory containing the application entry script file. The property {@link setBaseUrl baseUrl} refers to the URL
 * for accessing the {@link setBasePath basePath}.
 *
...
class CAssetManager extends CApplicationComponent
{
     ...
     * Defaults to exclude '.svn' and '.gitignore' files only. This option has no effect if {@link linkAssets} is
     * enabled.
     * @since 1.1.6
     **/
     ...
     *                            If false, the name will be the hash taken from dirname of the path being published
     *                            and path mtime. Defaults to false. Set true if the path being published is shared
     ...
     *                            assets are being constantly changed. The consequence is that the performance is
     *                            degraded, which is not a concern during development, however. Default value of this
     *                            parameter is null meaning that it's value is controlled by {@link $forceCopy} class
     *                            property. This parameter has been available since version 1.1.2. Default value has
     *                            been changed since 1.1.11. Note that this parameter cannot be true when {@link
     *                            $linkAssets} property has true value too. Otherwise an exception would be thrown.
     *                            Using this parameter with {@link $linkAssets} property at the same time is illogical
     *                            because both of them are solving similar tasks but in a different ways. Please refer
