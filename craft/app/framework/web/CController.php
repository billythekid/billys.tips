 ...
 * @property string     $viewPath     The directory containing the view files for this controller. Defaults to
...
class CController extends CBaseController
{
     ...
     * @param array  $config          the action configuration that should be applied on top of the configuration
     ...
     * @param string $basePath       the directory that is used to search for an absolute view name under the
     *                               application
     * @param string $moduleViewPath the directory that is used to search for an absolute view name under the current
     *                               module. If this is not set, the application base view path will be used.
     ...
     *                       with module ID (optional in the current module), controller ID and action ID. If the
     *                       former, the action is assumed to be located within the current controller.
     ...
     *                           the current controller class. If the method does not exist, it is assumed to be a
     *                           global PHP function. Note, the callback should return the dynamic content instead of
     *                           echoing it.
     */
     ...
     *                          If the ControllerID is not present, the current controller ID will be prefixed to the
     *                          route. If the route is empty, it is assumed to be the current action. If the controller
     *                          belongs to a module, the {@link CWebModule::getId module ID} will be prefixed to the
     *                          route. (If you do not want the module ID prefix, the route should start with a slash
     *                          '/'.)
     ...
     *
     * @param string $route     the URL route. This should be in the format of 'ControllerID/ActionID'.
     *                          If the ControllerPath is not present, the current controller ID will be prefixed to the
     *                          route. If the route is empty, it is assumed to be the current action.
     * @param array  $params    additional GET parameters (name=>value). Both the name and value will be URL-encoded.
     * @param string $schema    schema to use (e.g. http, https). If empty, the schema used for the current request
     *                          will be used.
     * @param string $ampersand the token separating name-value pairs in the URL.
     * @return string the constructed URL
     */
     ...
     * @param boolean $terminate  whether to terminate the current application after calling this method. Defaults to
     *                            true.
     ...
     * This filter throws an exception (CHttpException with code 400) if the applied action is receiving a non-POST
     * request.
     *
     ...
     * This filter throws an exception (CHttpException with code 400) if the applied action is receiving a non-AJAX
     * request.
     *
     * @param CFilterChain $filterChain the filter chain that the filter is on.
