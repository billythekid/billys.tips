...
class CWebUser extends CApplicationComponent implements IWebUser
{
     ...
     * When a request is made and user session has expired, {@link loginRequired} redirects to {@link loginUrl} for
     * login. If that happens during an ajax call, the complete HTML login page is returned as the result of that ajax
     * call. That could be a problem if the ajax call expects the result to be a json array or a predefined string, as
     * the login page is ignored in that case. To solve this, set this property to the desired return value. If this
     * property is set, this value will be returned as the result of the ajax call in case that the user session has
     * expired.
     * @since 1.1.9
     * @see   loginRequired
     */
     ...
     *                                meaning login till the user closes the browser. If greater than 0, cookie-based
     *                                login will be used. In this case, {@link allowAutoLogin} must be set true,
     ...
     *                                then {@link clearStates} will be called, which removes only the data stored via
     *                                {@link setState}.
     */
     ...
     *                              operation. If this parameter is false, this method will always call {@link
     *                              CAuthManager::checkAccess} to obtain the up-to-date access result. Note that this
