 ...
 ************************************* NOTES *************************************
 * The API Wrapper returns PHP array's (When experimenting with the class it can be helpful to use print_r to see the
 * structure of the returned array) Methods always use an array for parameters Errors are stored in a string parameter
 * of the API class ->errors Only GET requests can be cached, POST are never cached The default cache storage location
 * can be set by passing in cacheDir as a parameter to HelpSpotAPI()
 ************************************ EXAMPLES ***********************************
 * MOST BASIC USAGE
 ...
 * Desc: Create a request via the public api which does not require a password. This is the same as creating a request
 * via the portal. include('HelpSpotAPI.php');
 * $hsapi = new HelpSpotAPI(array("helpSpotApiURL" => "http://.../api/index.php"));
 ...
 * Desc: This uses the private API to create a request which opens up many more possibilities for populating fields,
 * assigning the request and more include('HelpSpotAPI.php');
