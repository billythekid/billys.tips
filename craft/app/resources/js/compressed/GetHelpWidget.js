...
(function (b) {
    Craft.GetHelpWidget = Garnish.Base.extend({
        ...
        init: function (a) {
            ...
            this.addListener(this.$sendBtn, "activate",
                "sendMessage")
        },
        sendMessage: function () {
            var a = "iframeWidget" + this.widgetId;
            this.loading || (this.$iframe || (this.$iframe = b('<iframe id="' + a + '" name="' + a + '" style="display: none" />').insertAfter(this.$form)), this.loading = !0, this.$sendBtn.addClass("active"), this.$spinner.removeClass("hidden"), this.$form.attr("target", a), this.$form.attr("action", Craft.getActionUrl("dashboard/sendSupportRequest")), this.$form.submit())
        },
        parseResponse: function (a) {
            ...
            if (a.errors) {
                this.$errorList || (this.$errorList = b('<ul class="errors"/>').insertAfter(this.$form));
                for (var d in a.errors)for (var c = 0; c < a.errors[d].length; c++)b("<li>" + a.errors[d][c] + "</li>").appendTo(this.$errorList)
            }
            a.success && (Craft.cp.displayNotice(Craft.t("Message sent successfully.")), this.$message.val(""), this.$attachAdditionalFile.val(""));
            this.$iframe.html("")
        }
    }, {widgets: {}})
})(jQuery);
