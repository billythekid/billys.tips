...
class HttpRequestService extends \CHttpRequest
{
     ...
     *                                  Defaults to `null`.
     * @return mixed The value of the corresponding query string param if a single param was requested, or
     *               $defaultValue
     *                                  if that value didn’t exist, or the entire $_GET array if no single param was
     *                                  requested.
     */
     ...
     *                                  if that value didn’t exist, or the entire $_POST array if no single param was
     *                                  requested.
     */
     ...
     * This method will first search for the given paramater in the query string, calling {@link getQuery()}
     * internally,
     ...
     * $name can also represent a nested parameter using a dot-delimited string.
     * ```php
     * $bar = craft()->request->getParam('foo.bar'); // Returns $_GET['foo']['bar'] or $_POST['foo']['bar'], if either
     * exist
     * ```
     * All values will be converted to UTF-8, regardless of the original character encoding.
     *
    ...
    /**
     * Returns the cookie collection. The result can be used like an associative array. Adding {@link HttpCookie}
     * objects to the collection will send the cookies to the client; and removing the objects from the collection will
     * delete those cookies on the client.
     *
    ...
    /**
     * Returns the path Craft should use to route this request, including the [CP
     * trigger](http://craftcms.com/docs/config-settings#cpTrigger) if it is in there.
     *
