...
/**
 * ConfigService provides APIs for retrieving the values of Craft’s [config
 * settings](http://craftcms.com/docs/config-settings), as well as the values of any plugins’ config settings. An
 * instance of ConfigService is globally accessible in Craft via {@link WebApp::config `craft()->config`}.
 *
...
class ConfigService extends BaseApplicationComponent
{
     ...
     * then only a single value in that array will be returned: the one that has a key matching the `$localeId`
     * argument. If no matching key is found, the first element of the array will be returned instead. This function is
     * used for Craft’s “localizable” config settings:
     ...
     *
     * @param string $item    The name of the config setting.
     * @param mixed  $default The default value to be returned in the event that the config setting isn’t set. Defaults
     *                        to `null`.
     ...
     *                     returns a full **URL**. And in the event that you do just want the path (as the name of the
     *                     method implies), you have to pass in a $code and $uid just to be able to set the 4th
     *                     argument to `false`, even though those variables will won't be used.
     * @todo     Create a new getActivateAccountUrl() method (probably elsewhere, such as UrlHelper) which handles
     *                     everything that setting $full to `true` currently does here. The function should accept an
     *                     actual UserModel rather than their UID, for consistency with {@link getSetPasswordPath()}.
     *                     Let this function continue working as a wrapper for getActivateAccountUrl() for the time
     *                     being, with deprecation logs.
     */
     ...
     * @internal This is a little awkward in that the method is called getActivateAccount**Path**, but it's also
     *           capable
     *                        of returning a full **URL**. And it requires you pass in both a user’s UID *and* the
     *                        UserModel - presumably we could get away with just the UserModel and get the UID from
     *                        that.
     * @todo     Create a new getSetPasswordUrl() method (probably elsewhere, such as UrlHelper) which handles
     *                        everything that setting $full to `true` currently does here. The function should not
     *                        accetp a UID since that's already covered by the UserModel. Let this function continue
     *                        working as a wrapper for getSetPasswordUrl() for the time being, with deprecation logs.
     */
    ...
    /**
     * Parses a string for any [environment
     * variables](http://craftcms.com/docs/multi-environment-configs#environment-specific-variables). This method
     * simply loops through all of the elements in the
